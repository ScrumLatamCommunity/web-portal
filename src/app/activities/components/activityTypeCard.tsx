...
export default function ActivityTypeCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative ml-2 h-44 w-[95%] transform cursor-pointer overflow-hidden rounded-2xl shadow-lg transition-all duration-300 md:ml-0 md:w-full lg:h-60 lg:max-w-[420px] ${
    ...
    >
      ...
      >
        <div>
          <h4 className='font-darker-grotesque text-xl font-bold sm:text-2xl lg:text-xl'>
            {category.title}
          </h4>

          <p
            className={`mt-2 max-h-0 overflow-hidden text-base text-white/80 opacity-0 transition-all duration-300 ease-in-out group-hover:max-h-40 group-hover:opacity-100 lg:text-base ${active ? 'max-h-40 opacity-100' : ''} `}
          >
