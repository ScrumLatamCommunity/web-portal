...
export default function SearchBar() {
  ...
  const [filteredNews, setFilteredNews] = useState<Blog[]>([])
  const dropdownRef = useRef<HTMLDivElement | null>(null)

  const handleSearch = () => {
    const results = blogs.filter((blogsItem) =>
      blogsItem.title.toLowerCase().includes(query.toLowerCase()),
    )
            ...
            >
              ...
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: highlightMatch(blogsItem.title, query),
                }}
              />
            </div>
          ))}
